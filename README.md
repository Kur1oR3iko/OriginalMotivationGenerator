...
三指下划时，短语页整体向下翻出，“kurio最新力作（2）”设置页从上方翻入，切换约 0.24 秒。三指上划或点击“返回短语”反向翻回，系统开启“减弱动态效果”时直接换页。设置页提供软件介绍和“时间的流速”模式，自动切换间隔可选 0.5 秒、5 秒、10 秒、30 秒、1 分钟、5 分钟，设置会自动保存。打开设置或应用进入后台时暂停，返回后重新计时；点击短语会生成下一条并关闭自动切换。Mac 上可点击右上角设置按钮或按 `⌘,` 打开设置。
